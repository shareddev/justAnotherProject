...
public class DFS<T> extends CommonSearcherAbstract<T> {
	...
	// left to check
	private ArrayList<State<T>> possibleNeighbors = new ArrayList<State<T>>();
	...
	public Solution search(ISearchable<T> searchable) {
		...
		while (!stack.isEmpty()) {
			...
			possibleNeighbors = new ArrayList<State<T>>(
					Integer.valueOf((searchable.getAllStates(currentState).toString())));
	...
	private boolean isContainsNeighbor(State<T> state, ArrayList<State<T>> explored) {
