...
public class MyCacheManager implements ICacheManager {
	...
	// C'Tor
	// Because the MyClientHandler sends a file path by default, we don't need to
	// concern with a
	...
	public void loadSolutions() {
		...
		for (int i = 0; i < filesArray.length; i++) {
			try {
				this.solutions.put(filesArray[i].getName().substring(0, filesArray[i].getName().length() - 4),
